...
function updateUrlWithSessionId(sessionId) {
    const newUrl = `${window.location.origin}/apps/ballgame?session_id=${sessionId}`;
...
if (copySessionUrlBtn) {
    copySessionUrlBtn.addEventListener('click', () => {
        if (currentSessionId) {
            const sessionUrl = `${window.location.origin}/apps/ballgame?session_id=${currentSessionId}`;
